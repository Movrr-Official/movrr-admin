...
export async function getSessionAnalytics(days = 30): Promise<{
  ...
  try {
    ...
    // Fetch bonus breakdown from reward_transaction metadata
    ...
    rows.forEach((row) => {
      ...
      if (!point) return;
      const status = verMap.get(row.id) ?? "pending";
      if (status === "verified") point.verified++;
      else if (status === "rejected") point.rejected++;
      else if (status === "manual_review") point.manual_review++;
      else point.pending++;
    });
    ...
    rows.forEach((row) => {
      ...
      if (!entry) return;
      entry.meters +=
        Number((row as Record<string, unknown>).total_distance_meters ?? 0);
      entry.sessions++;
    });

    const distanceTrend: DistanceTrendPoint[] = Array.from(distMap.entries()).map(
      ([date, { meters, sessions }]) => {
    ...
    (txRows ?? []).forEach((tx) => {
      ...
      if (!meta) return;
      const breakdown = meta.bonus_breakdown as Array<{
        type: string;
        addedPoints?: number;
        multiplier?: number;
      }> | undefined;
      ...
      breakdown.forEach((entry) => {
        ...
        existing.points += entry.addedPoints ?? 0;
        existing.count++;
        bonusAccum.set(entry.type, existing);
      });
    });

    const bonusBreakdown: BonusTypeBreakdown[] = Array.from(bonusAccum.entries())
    ...
    return {
      success: true,
      data: { verificationTrend, bikeTypeDistribution, distanceTrend, bonusBreakdown },
    };
  } catch (error) {
    return {
      success: false,
...
export async function getStreakLeaderboard(limit = 10): Promise<{
  ...
  try {
    ...
    const { data: txRows, error } = await supabase
      ...
      .in("source", ["bonus", "standard_ride_bonus", "ad_boost"])
      .not("metadata", "is", null)
    ...
    (txRows ?? []).forEach((tx) => {
      const meta = tx.metadata as Record<string, unknown> | null;
      if (!meta) return;
      const breakdown = meta.bonus_breakdown as Array<{ type: string; addedPoints?: number }> | undefined;
      if (!Array.isArray(breakdown)) return;
      const streakEntries = breakdown.filter((e) => e.type === "streak_bonus");
      if (!streakEntries.length) return;
      const riderId = tx.rider_id as string;
      if (!riderId) return;
      const streakPts = streakEntries.reduce((s, e) => s + (e.addedPoints ?? 0), 0);
    ...
    const userIds = (riderRows ?? []).map((r) => r.user_id).filter(Boolean) as string[];
    const riderToUser = new Map((riderRows ?? []).map((r) => [r.id, r.user_id]));
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to fetch streak leaderboard",
    };
  }
}
